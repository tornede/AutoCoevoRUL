...
public class MedianFeatureRater implements IFeatureRater {
	...
	public double rateFeatureExtractor(final List<GGPSolutionCandidate> solutionCandidatesWithFeatureExtractor) {
		double[] optionalResult = solutionCandidatesWithFeatureExtractor.stream().filter(candidate -> candidate.getScore() != null && !candidate.getScore().isNaN())
				.mapToDouble(candidate -> candidate.getScore()).sorted().toArray();
