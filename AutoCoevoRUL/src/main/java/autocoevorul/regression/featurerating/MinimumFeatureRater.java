...
public class MinimumFeatureRater implements IFeatureRater {

	@Override
	public double rateFeatureExtractor(final List<GGPSolutionCandidate> solutionCandidatesWithFeatureExtractor) {
		OptionalDouble optionalResult = solutionCandidatesWithFeatureExtractor.stream().filter(candidate -> candidate.getScore() != null && !candidate.getScore().isNaN()).mapToDouble(candidate -> candidate.getScore()).min();
