...
public class BinaryAttributeSelectionIncludedGenomeHandler extends GenomeHandler {
	...
	protected void setupAdditionalComponents(final ExperimentConfiguration experimentConfiguration) throws ExperimentEvaluationFailedException {
		for (IAttribute attribute : experimentConfiguration.getTrainingData().getListOfAttributes()) {
			String genome_name = getAttributeNameForGenomeRepresentation(attribute);
	...
	private String getAttributeNameForGenomeRepresentation(IAttribute attribute) {
		return "attribute_"+attribute.getName();
	}

	@Override
	public List<Solution> getInitialSolutions(FeatureExtractionMoeaProblem problem, int populationSize)
	...
	public Solution getEmptySolution(final FeatureExtractionMoeaProblem problem) {
		...
		for (IComponent component : mainComponents) {
			if (!mainComponentNamesWithoutActivationBit.contains(component.getName())) {
	...
	public Solution activateTsfresh(Solution solution) {
		...
		for (IComponent component : allComponents.stream().filter(c -> c.getName().contains("TsfreshFeature")).sorted((c1, c2) -> c1.getName().compareTo(c2.getName())).collect(Collectors.toList())) {
